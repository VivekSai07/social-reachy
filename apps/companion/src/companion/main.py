...
from companion import app_lifecycle
...
if TYPE_CHECKING:
    from companion.console import LocalStream
...
def main() -> None:
    ...
    if args.command == "tool-spaces":
        from companion.tool_spaces import handle_tool_spaces_command
...
def run(
    ...
    from companion import sessions
    from companion.memory import list_memory_facts
    from companion.moves import MovementManager
    ...
    from companion.startup_settings import (
        StartupSettings,
    ...
    from companion.console import LocalStream
    from companion.tools.core_tools import ToolDependencies, initialize_tools
    from companion.conversation_handler import ConversationHandler
    ...
    session_id, session_start_ts = sessions.start_session(instance_path)
    ...
    def build_handler(startup_voice: Optional[str] = None) -> ConversationHandler:
        ...
        from companion.huggingface_realtime import HuggingFaceRealtimeHandler
    ...
    def go_to_sleep_and_stop_app() -> dict[str, Any]:
        ...
        try:
            ...
            try:
                new_facts = [f.text for f in list_memory_facts(instance_path) if f.created_at >= session_start_ts]
                sessions.end_session(session_id, "; ".join(new_facts), instance_path)
            except Exception as e:
    ...
    try:
        ...
        # Safety net for exit paths that bypass go_to_sleep_and_stop_app()
        # (dashboard stop, app switch, crash): end_session() is idempotent,
        # a no-op if the session was already closed above.
        try:
            sessions.end_session(session_id, "", instance_path)
        except Exception as e:
            logger.debug(f"Error finalizing session record during shutdown: {e}")

        time.sleep(1)
        logger.info("Shutdown complete.")


class Companion(ReachyMiniApp):  # type: ignore[misc]
