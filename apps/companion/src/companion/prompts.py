...
from companion.config import DEFAULT_PROFILES_DIRECTORY, config, get_default_voice
from companion.memory import format_memory_for_prompt
from companion.sessions import format_session_context_for_prompt
...
def get_session_instructions(instance_path: str | Path | None = None) -> str:
    ...
    context_parts = [
        part
        for part in (
            format_session_context_for_prompt(instance_path),
            format_memory_for_prompt(instance_path),
        )
        if part
    ]
    if context_parts:
