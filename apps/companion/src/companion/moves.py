...
from companion.gaze import GazeConfig, GazeController
...
# Idle-move types: mutually exclusive with each other (only one active at a
# time), and with every other primary move. Used to generalize the eviction
# checks below without hardcoding BreathingMove/GazeMove at each call site.
_IDLE_MOVE_TYPES = (BreathingMove, GazeMove)
...
class MovementManager:
    ...
    def __init__(
        ...
        # Webcam-based gaze tracking (companion's own, distinct from the
        # daemon-side `_head_tracking` above -- see gaze_tracking tool).
        # Construction is I/O-free; only WebcamFaceTracker.start()/.stop()
        # touch the camera. Long-lived and shared across every GazeMove
        # activation -- see gaze_move.py's docstring for why that matters.
        self._gaze_tracking_enabled = False
        self._gaze_controller = GazeController(GazeConfig())
        self._webcam_tracker = WebcamFaceTracker()
    ...
    def set_gaze_tracking(self, enabled: bool) -> None:
        """Start or stop following the user's face via the webcam-based tracker.

        Thread-safe via the command queue. Distinct from set_head_tracking:
        this drives a GazeMove through the normal move queue/current_move
        slot (so it pauses like breathing whenever a real move plays),
        rather than the daemon-side tracking side-channel.
        """
        self._command_queue.put(("set_gaze_tracking", enabled))
    ...
    def _handle_command(self, command: str, payload: Any, current_time: float) -> None:
        ...
        elif command == "set_gaze_tracking":
            enabled = bool(payload)
            if self._gaze_tracking_enabled == enabled:
                return
            self._gaze_tracking_enabled = enabled
            if enabled:
                # WebcamFaceTracker.start() is idempotent (no-op if already
                # running). Deliberately NOT calling .stop() on disable: it
                # joins its capture thread (up to 2s) and would block this
                # worker thread -- the same one that must call set_target()
                # every ~16.7ms -- freezing the robot for the join duration.
                # The tracker instead keeps running harmlessly in the
                # background once started; nothing reads its output while
                # gaze tracking is disabled. It's only ever actually stopped
                # in MovementManager.stop(), which runs outside this loop.
                self._webcam_tracker.start()
            # Both BreathingMove and GazeMove have an infinite duration, so
            # without proactively evicting here, flipping this flag while the
            # robot is already idling would sit inert until an unrelated real
            # move happened to interrupt whichever one is currently running.
            if isinstance(self.state.current_move, _IDLE_MOVE_TYPES):
                self.state.current_move = None
                self.state.move_start_time = None
                self._breathing_active = False
    ...
    def _manage_move_queue(self, current_time: float) -> None:
        ...
        if self.state.current_move is None or (
            ...
            if self.move_queue:
                ...
                # Any real move cancels the idle (breathing/gaze) mode flag
                self._breathing_active = isinstance(self.state.current_move, _IDLE_MOVE_TYPES)
        ...
        if (
            ...
            if idle_for >= self.idle_inactivity_delay:
                try:
                    ...
                    if self._gaze_tracking_enabled:
                        idle_move: Move = GazeMove(
                            gaze_controller=self._gaze_controller,
                            webcam_tracker=self._webcam_tracker,
                            interpolation_start_pose=current_head_pose,
                            interpolation_start_antennas=current_antennas,
                            interpolation_duration=0.5,
                        )
                        logger.debug("Started gaze tracking after %.1fs of inactivity", idle_for)
                    else:
                    ...
                    self._breathing_active = False
                    logger.error("Failed to start idle move: %s", e)

        if isinstance(self.state.current_move, _IDLE_MOVE_TYPES) and self.move_queue:
            self.state.current_move = None
            self.state.move_start_time = None
            self._breathing_active = False
            logger.debug("Stopping idle move due to new move activity")

        if self.state.current_move is not None and not isinstance(self.state.current_move, _IDLE_MOVE_TYPES):
            self._breathing_active = False
