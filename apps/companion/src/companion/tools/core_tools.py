...
from companion.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from companion.config import config
from companion.mcp_client import McpToolTimeoutError, McpToolInvocationError
from companion.tool_spaces import build_remote_client, read_installed_tool_spaces
from companion.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from companion.mcp_client import RemoteMcpToolClient
    from companion.tools.background_tool_manager import BackgroundToolManager
...
def _load_profile_tools(tool_names: list[str], remote_tool_names: set[str]) -> List[type[Tool]]:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"companion.tools.{tool_name}"
            try:
